...
int rAdventurer(struct gameState *state){
  int z = 0;
  int drawntreasure = 0;
  int cardDrawn;
...
int rCouncil_room(struct gameState *state, int handPos){
  int i;
  int currentPlayer = whoseTurn(state);
...
drawCard(currentPlayer, state);
  ...
  //+1 Buy
  state->numBuys = state->numBuys + 10;
...
int rFeast(struct gameState *state, int choice1){
  int i;
  int x;
  int currentPlayer = whoseTurn(state);
  int temphand[MAX_HAND];
...
int rSmithy(struct gameState *state, int handPos){
  int i;
  int currentPlayer = whoseTurn(state);
...
int rGreat_hall(struct gameState *state, int handPos){
  int currentPlayer = whoseTurn(state);
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card )
    {
    case adventurer:
      return rAdventurer(state);

    case council_room:
      return rCouncil_room(state, handPos);

    case feast:
      return rFeast(state, choice1);
    ...
    case great_hall:
      return rGreat_hall(state, handPos);
